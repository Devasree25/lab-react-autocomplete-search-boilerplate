...
const CountryList = () => {
  ...
  useEffect(() => {
    let filtered = []
    // Filter countries based on the first letter of the name
    if (searchText== ""){
      filtered = []
    }else{
      filtered = countriesData.filter(
        (country) =>
          ...
          country.name.toLowerCase().indexOf(searchText.toLowerCase()) === 0
      );
    }
  ...
  const handleSearchButtonClick = () => {
    if (filteredCountries.length > 0) {
      setSearchResult(filteredCountries[0].name);
    } else {
  ...
  return (
    <div className='box'>
      ...
      <div>
        {searchResult && <p>Searched word: {searchResult}</p>}
      </div>
      <div>
        {/* Display filtered countries based on the typed letter */}
        <ul style={ulStyle} >
          {filteredCountries.map((country) => (
            <li key={country.code}>{country.name}</li>
          ))}
        </ul>
        {filteredCountries.length === 0 && searchText && (
          <p>No result found</p>
        )}
      </div>
    </div>
